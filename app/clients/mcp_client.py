...
def build_kb_search_tool_proxy(client: MultiServerMCPClient) -> BaseTool:
    ...
    **без сетевого обращения к MCP Tools Server** — `.bind_tools()` нужна
    ...
    async def vera_rag_kb(query: str) -> dict:
        """Юридический поиск по базе знаний Vera о трудоустройстве и трудовых правах.

        Вызывай для правовых и иных проверяемых фактов. query — самодостаточный
        поисковый запрос. Самодостаточное текущее
        сообщение передавай в query дословно, без переформулировки и новых
        фактов. Контекст добавляй только из фактических предыдущих сообщений
        этой же сессии. Не добавляй инвалидность, роль или аудиторию из
        специализации Веры. Если сообщение состоит только из ссылки на норму,
        передавай её символ в символ и не расшифровывай содержание. При неполной
        реплике без однозначного антецедента сначала уточни предмет. Уточнение
        приоритетнее вызова: не вызывай инструмент с дополненным догадкой query,
        а отдельного тематического слова недостаточно для самодостаточности.
        Пустой chunks означает, что поиск не нашёл ответа, а не техническую ошибку.
        """
