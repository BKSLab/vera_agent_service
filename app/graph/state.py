...
class AgentState(TypedDict):
    ...
    consultation_email_guard_notice: str | None
    """`analyze_intent` выставляет непустую инструкцию (VERA-021), если код
    отклонил предложенный моделью вызов `send_consultation_email` (нет
    кодового подтверждения по истории). `generate_direct` добавляет её
    отдельным `SystemMessage` только для этого вызова модели — не в
    `state['messages']` — чтобы модель не выдумывала текстовый псевдовызов
    инструмента, думая, что отправка уже идёт. Как и `search_unavailable`,
    не участвует в накоплении между репликами: каждый новый вызов графа
    должен явно передавать `None`."""
