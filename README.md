...
**P0 — блокирует полностью, без этого разворачивать бессмысленно:**
- **MCP Tools Server не существует** — отдельный трек, ещё не начат. Пока его нет, `kb_search` всегда деградирует в «поиск недоступен» (раздел 0.1 плана) — агент поднимется и будет отвечать, но не по существу вопросов из базы знаний. Это единственная причина, по которой реальный запуск сейчас в принципе не имеет смысла, всё остальное ниже — уже второй эшелон.
- **LLM-креды — плейсхолдеры** в `.env` (`LLM_API_KEY=your_llm_api_key` и т.п.) — нужен реальный OpenAI-совместимый провайдер (`AGENT_VERA_ARCHITECTURE.md`: «провайдер конфигурируется», конкретный выбор не зафиксирован).

**P1 — инфраструктура сейчас dev-уровня, не прод:**
- RabbitMQ/Redis в `docker-compose.yml` — дефолтные пароли (`changeme`), без внешнего резервирования/кластеризации.
...
- Полный путь `Agent → MCP → RAG` с реальным контентом никогда не прогонялся целиком, даже гипотетически — упирается в P0.
