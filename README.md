...
Содержимое диалога в трейсы не попадает (VERA-022): тексты входа и выхода,
системные промпты и история сообщений скрыты `trace_content_enabled` (по
умолчанию `false`), а у поисковой тулы вместо запроса и результата
сохраняются только длина запроса и число найденных чанков. Именно поэтому
порт Phoenix опубликован наружу — включение `trace_content_enabled` вернёт
тексты в трейсы, и порт придётся закрывать: аутентификации у Phoenix нет.
