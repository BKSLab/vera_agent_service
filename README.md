...
СНИЛС и паспортные данные локально заменяются request-local маркерами. ФИО
...
перед вызовом `send_consultation_email`; модель видит лишь маркер. Исходный
вопрос по-прежнему сохраняется в PostgreSQL и корневом Phoenix-span согласно
описанным выше настройкам, поэтому эта защита относится именно к границе LLM.
