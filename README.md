...
Перед внешним LLM-вызовом распознанные ФИО, email, российские телефоны,
СНИЛС и паспортные данные локально заменяются сессионными маркерами. ФИО
извлекается каскадом: строгие контекстные маркеры и морфология Yargy, затем
локальный Natasha NER; это покрывает lower-case ввод без слепого правила
`я + любое слово`.
Для отправки консультации email восстанавливается только непосредственно
перед вызовом `send_consultation_email`; без активного контекста или для
неизвестного маркера отправка блокируется. Только значения из пользовательских
сообщений сохраняются в сессионном mapping и разрешаются для отправки; данные
из ответов модели и инструментов маскируются, но не авторизуются. Соответствия
маркеров сохраняются в Redis-checkpoint с TTL сессии, но не включаются в
сообщения для модели.
В аргументах `send_consultation_email` маркер разрешён только в поле `email`.
Модель обязана сформировать текст и тему письма без маркеров; если маркер всё
же остался, Agent Service перед MCP заменяет его нейтральной формулировкой без
восстановления исходного значения и пишет безопасный warning без содержимого
письма и ПДн.
Исходный вопрос по-прежнему сохраняется в PostgreSQL и корневом Phoenix-span
согласно описанным выше настройкам, поэтому эта защита относится именно к
границе LLM.
